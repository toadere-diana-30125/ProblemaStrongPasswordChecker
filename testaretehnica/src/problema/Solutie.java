...
public class Solutie {
    ...
    public  int strongPasswordChecker(String s) {
        ...
        // Contorizăm numărul de  "adăugări"(inserări) si frecvența caracterelor întalnite
        readString();

        // Folosirea ștergerilor pentru a minimiza pauzele necesare
        if (s.length() > 20) spendDeletions();

        // numărul de înlocuiri în secvență necesare
        int numpauze = 0;
        ...
        // pentru secvențe scurte ,este necesară consolidarea prin inserții .
        if (s.length() < 6) {
            int numarinsertii = 6 - s.length();
            numschimbari = Math.max(numarinsertii, numschimbari);
        }

        // Pentru secvențe prea lungi, adăugăm numărul de ștergeri si înlocuiri necesare
        ...
        for (int i = lastThreeMult; i < lastThreeMult + 3; i++) {
            ...
            while (j > 2 && ndtemp > 0) {
                if (seq[j] > 0) {
                    ...
                    // analizăm dacă mai avem trebuie să utilizăm ștergerile în continuare
                    ndtemp -= d;
                }
                else j -= 3;
            }
